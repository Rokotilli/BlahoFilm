    ...
    public class OutboxMessagesBackgroundService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    var dbContext = scope.ServiceProvider.GetRequiredService<SeriesServiceDbContext>();
                    ...
                    {
                        var series = item.Data as Series;

                        try
                        {
                            await publishEndpoint.Publish(new MediaRegisteredMessage() { Id = series.Id, MediaType = MediaTypes.Series }, new CancellationTokenSource(TimeSpan.FromMinutes(1)).Token);
                        }
                        catch { break; }
